...
module.exports = new class {
    ...
    purpose2usage( as, purpose ) {
        switch ( purpose ) {
        case 'MAC': return [ 'sign', 'temp' ];
        case 'ENC': return [ 'encrypt', 'temp' ];
        case 'EXPOSED': return [ 'encrypt', 'sign', 'temp' ];
        default: as.error( Errors.Invoker, `Unknown key purpose: ${purpose}` );
        }
    }

    ensureDerivedKey( as, ccm, purpose, { msid, type, kds, salt, prm } ) {
    ...
    encryptKey( as, ccm, key_id, { salt, msid, type, kds, prm, mode } ) {
    ...
    injectEncryptedMACKey(
        as, ccm,
        { ext_id, ekey, local_id, global_id },
        { salt, msid, type, kds, prm, mode }
    ) {
        this.ensureDerivedKey(
            as, ccm, 'ENC',
            { salt, msid, type, kds, prm }
