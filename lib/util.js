...
module.exports = new class {
    ...
    reqinfo2source( reqinfo ) {
        const res = {};
        const channel = reqinfo.channel();
        const chan_type = channel.type();

        if ( chan_type === 'HTTP' ) {
            // User Agent
            const headers = channel.getRequestHeaders();
            ...
            if ( 'cookie' in headers ) {
                // Enable heavy processing only for requests with cookie set
                const client_token = channel.getCookie( CLIENT_TOKEN_COOKIE );

                if ( client_token ) {
                    res.client_token = client_token;
                }
            }
        }

        // Source IP
        const { host } = reqinfo.info.CLIENT_ADDR;
